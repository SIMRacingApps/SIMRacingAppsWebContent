'use strict';
define(['angular','angular-animate','angular-sanitize','angular-chart','css!SIMRacingApps'],
function(angular) {
    /**
     ...
     */
    var app = {module: angular.module('SIMRacingApps',['ngSanitize','ngAnimate','chart.js'])};
     ...
     * @ngdoc filter
     * @since 1.8
     * @param {Object} value The value as a Date object of Milliseconds since Epoch
     * @return {Object} The results of calling date
     * @author Jeffrey Gilliam
     */
    app.sraDate = function($filter) {
        return function(value,format,timezone) {
            var t = timezone == 'CEST' ? '+0200' : timezone;
            var s = $filter('date')(value,format,t);
            return s;
        }
    };
    app.module.filter('sraDate', ['$filter',app.sraDate] );
    ...
    app.sraDuration = function($filter,  $locale) {
        return function(time,precision,showPositive,blankIfZero,showMinutes,showApproximateOnly) {
            ...
            if (time && angular.isDefined(time.Value)) {
                dur = time.Value;
                uom = $filter('sraEncodeEntities')(time.UOMAbbr);
...
//dur = (60*60*24*5) + (60*60*4) + (60*3) + 2;
     ...
     * @param {Boolean} preserveBR Default false. True will not encode the &lt;BR /&gt; tag.
     * @return {String} A string with entities encoded. 
     * @author Jeffrey Gilliam
     */
    app.sraEncodeEntities = function() {
        return function(str,preserveBR) {
        
            if (str && typeof str === 'string') {
                var s = (preserveBR ? str.replace(/[<][bB][rR][ ]*\/[>]/g,"xBRBRBRx") : str)
                        .replace(/&/g,"&amp;")
                        .replace(/</g,"&lt;")
                        .replace(/>/g,"&gt;")
                        .replace(/'/g,"&#39;")
                        .replace(/"/g,"&quot;")
                        .replace(/xBRBRBRx/g,"<BR />");
                return s;
            }
            return "";
        }
    };
    app.module.filter('sraEncodeEntities',[ app.sraEncodeEntities ]);
     ...
     * string with either the char passed in or a space according to the length passed in.
     * @ngdoc filter
     ...
     * @param {String}  char The string to prefix, default to space.
    ...
    app.sraNumber = function($filter,  $locale) {
        return function(value,decimals,blankIfZero,showPositive,showThousands,showApproximateOnly) {
            ...
            if (value && angular.isDefined(value.Value)) {
                ...
                uom = $filter('sraEncodeEntities')(value.UOMAbbr);
            ...
            if (uom.toUpperCase() == 'STRING' || !(v+"").match(/^[\-\+0-9]/g))
                return formatted;
    ...
    app.sraRound = function() {
        return function(value,decimalPlaces) {
            var multiplier = Math.pow(10.0,decimalPlaces||0);
     ...
     * string with either the char passed in or the space according to the length passed in.
     * @ngdoc filter
     ...
     * @param {String}  char The string to append, default to space.
...
//                    }
                ...
                processData: function(data) {
                    ...
                    //TODO: Find a way to only call sraOnResize when needed. For now only call every so often
                    
                    //2022.07.29 - Decided to use 5 seconds instead of 1 second to reduce the overhead found in some widgets, like the Track Map.
                    //             The side effect I saw, was some objects would not adjust to their text content until this was called.
                    //             Hopefully this timing is acceptable.
                    if ($rootScope.sraOnResize && (self.lastResize + 5000) < Date.now()) {
...
(function(i,s,o,g,r,a,m){i['GoogleAnalyticsObject2']=r;i[r]=i[r]||function(){
    (i[r].q=i[r].q||[]).push(arguments)},i[r].l=1*new Date();a=s.createElement(o),
    m=s.getElementsByTagName(o)[0];a.async=1;a.src=g;m.parentNode.insertBefore(a,m)
    })(window,document,'script','https://www.googletagmanager.com/gtag/js?id=G-XG2W0LQDFR','gtag');
